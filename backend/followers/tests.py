...
class FollowersDetailViewTest(APITestCase):
    ...
    def test_followers_detail_view_get(self):
        ...
        response = FollowerDetail.as_view()(request, foreign_id=self.author2.id)
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(response.data["items"][0]["url"], "http://testserver/authors/" + str(self.author2.id)) # should it start with None?
        self.assertEqual(response.data["items"][0]["host"], "http://testserver")
